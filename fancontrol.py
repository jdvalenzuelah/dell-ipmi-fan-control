...
def _exec_raw_ipmi_cmd(payload: str) -> int:
...
def enable_fancontrol() -> int:
    return _exec_raw_ipmi_cmd(_manual_fancontrol_toggle_command(enabled=True))

def disable_fancontrol() -> int:
    return _exec_raw_ipmi_cmd(_manual_fancontrol_toggle_command(enabled=False))

def set_manual_fanspeed(speed: int) -> int:
    if 100 >= speed >= 0:
        return _exec_raw_ipmi_cmd(_fan_speed_command(hex(speed)))
    else:
